...
**Generated Component Rules** (enforced by system prompt in `server/index.ts`):
*These constraints apply ONLY to React components generated by AI (react-live output):*
...
- Application code (`src/components/*`, `src/App.tsx`) uses modular CSS (`src/App.css`)
- `className` binding with external stylesheets is standard for application UI
- Generated components are isolated from application styling (react-live sandbox)
