...
**Generated Component Constraints** (enforced by system prompt in server/index.ts)
*These rules apply ONLY to React components generated by AI (react-live output):*
...
**Application UI Rules** (for src/* files):
- Do: Use modular CSS files (e.g., App.css) for application styling
- Do: Use className with external CSS for application components (PromptInput, ComponentCard, etc.)
- Do: Keep application UI separate from generated component code
...
- Do not: Include CSS imports in generated component code (inline styles only for generated output)
...
- Application Styling: Modular CSS (`src/App.css`) for the IDE itself
- Generated Components: Inline styles only (enforced by react-live sandbox)
...
- **Role**: 메인 Claude가 작업한 코드를 범위 한정하여 독립 검증
- **Scope**: TDD 준수, 테스트 커버리지, 네이밍, 단일 책임, 숨은 버그 점검
- **Output**: 요약 피드백만 반환 (코드 수정 불가)
